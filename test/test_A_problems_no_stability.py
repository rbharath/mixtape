...
def test_objective():
    ...
    for dim in dims:
        a_prob = A_problem_no_stability(dim)
        (d_I_cds, I_cds, A_cds, A_T_cds) = a_prob.coords()
...
def test_A_solve_1():
    ...
    for dim in dims:
        ...
        # Call solver
        t_start = time.time()
        A = a_prob.solve(d, B, C, E, Q, tol=tol, search_tol=search_tol,
                            verbose=False)
        t_end = time.time()

        #print "A\n", A
        print "dim: ", dim
        print "norm(A, 2): ", np.linalg.norm(A, 2)
...
def test_A_solve_2():
    ...
    for dim in dims:
        ...
        # Call solver
        t_start = time.time()
        A = a_prob.solve(d, B, C, D, E, Q, tol=tol)
        t_end = time.time()
...
def test_A_solve_plusmin():
    n_features = 1
    a_prob = A_problem_no_stability(n_features)
...
def test_A_solve_muller_5():
    ...
    A = a_prob.solve(d, B, C, D, E, Q, tol=tol,
        disp=True, debug=False, verbose=False)
    print "A\n", A
    print "norm(A, 2)\n", np.linalg.norm(A, 2)
    assert np.linalg.norm(A,2) < 1
